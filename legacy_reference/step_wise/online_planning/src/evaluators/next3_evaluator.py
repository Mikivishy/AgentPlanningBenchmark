"""
Next3 Strategy Evaluator.

This module contains the Next3 evaluator that predicts the next three steps.
"""
...
from ..models.data_models import PredictionResultNext3, EvalResultNext3
...
class Next3Evaluator(BaseOnlinePlanningEvaluator):
    """Evaluator for next3 prediction strategy (predict next three steps)."""

    def __init__(self, *args, **kwargs):
        # Force prediction_mode to be next3
        kwargs["prediction_mode"] = "next3"
    ...
    def truncate_trajectory(
        ...
        """
        Truncate trajectory at middle position for next_3 prediction.
        Predict the next THREE assistant tool call steps (without tool execution results).
        ...
        Returns:
            tuple: (prefix, ground_truth_steps, remaining_steps)
                ...
                - ground_truth_steps: the next THREE steps as a complete unit (not split)
                - remaining_steps: all remaining steps after the three steps (for reference)

        Note: All three ground truth steps must be assistant steps (role="assistant"),
        ...
        if len(steps) <= 3:
            # Not enough steps for next3 prediction
            return "", "", ""
        ...
        if base_type == "framethinker":
            ...
            # We want to predict the next THREE assistant steps, NOT user steps
            ...
            if len(assistant_indices) < 3:
                # Not enough assistant steps for next3 prediction
                return "", "", ""

            # Find three consecutive assistant steps (allowing tool/user steps in between)
            # Start from middle and look for valid triples
            mid_idx = len(assistant_indices) // 2

            # Search for a triple of assistant steps where:
            # 1. No user steps between them
            # 2. They are reasonably close together
            valid_triple_found = False
            first_assistant_idx = None
            second_assistant_idx = None
            third_assistant_idx = None

            # Try from middle onwards
            for i in range(mid_idx, len(assistant_indices) - 2):
                idx1 = assistant_indices[i]
                idx2 = assistant_indices[i + 1]
                idx3 = assistant_indices[i + 2]

                # Check if there's any user step between idx1 and idx2, or between idx2 and idx3
                has_user_step = False
                for j in range(idx1 + 1, idx3):
                ...
                if not has_user_step:
                    ...
                    third_assistant_idx = idx3
                    valid_triple_found = True
                    break

            # If no valid triple found in latter half, try earlier triples
            if not valid_triple_found:
                for i in range(mid_idx - 1, -1, -1):
                    if i + 2 >= len(assistant_indices):
                        continue
                    idx1 = assistant_indices[i]
                    idx2 = assistant_indices[i + 1]
                    idx3 = assistant_indices[i + 2]

                    # Check if there's any user step between idx1 and idx3
                    has_user_step = False
                    for j in range(idx1 + 1, idx3):
                        if (
            ...
            # Combine the three ground truth steps into a single JSON array
            ground_truth_steps = json.dumps(
                [
                    steps[first_assistant_idx],
                    steps[second_assistant_idx],
                    steps[third_assistant_idx],
                ],
                ensure_ascii=False,
            )
            remaining_steps = (
                json.dumps(steps[third_assistant_idx + 1 :], ensure_ascii=False)
                if (third_assistant_idx + 1) < len(steps)
                else ""
            )

        elif base_type == "opencua":
            # OpenCUA: list of dicts with {"role": "assistant", "content": "...", "action": "..."}
            # Simple: take three consecutive steps from middle
            mid = len(steps) // 2
            if mid + 2 >= len(steps):
                return "", "", ""
            ...
            # Combine the three ground truth steps into a single JSON array
            ground_truth_steps = json.dumps(
                [steps[mid], steps[mid + 1], steps[mid + 2]], ensure_ascii=False
            )
            remaining_steps = (
                json.dumps(steps[mid + 3 :], ensure_ascii=False)
                if mid + 3 < len(steps)
                else ""
            )
            ...
            # Find assistant steps and predict next THREE assistant steps
            ...
            # Find three consecutive assistant steps where:
            # 1. No user steps between them
            # 2. They are reasonably close together
            mid_idx = len(assistant_indices) // 2
    ...
    def generate_prediction_prompt(self, sample: Dict, prefix: str) -> str:
        ...
        # Build dataset-specific output format instructions for THREE steps
        format_instruction = ""
        if base_type in ["gaia", "gta"]:
            format_instruction = """
**REQUIRED OUTPUT FORMAT:**
Your response MUST be a JSON array containing exactly THREE assistant steps:
```json
[
    {
        "role": "assistant",
    ...
    {
        "role": "assistant",
        "thought": "Your reasoning for the third step",
        "tool_calls": [
            {
                "type": "function",
                "function": {
...
[
    ...
    {
        "role": "assistant",
        "content": "Your reasoning for the third step",
        "tool_calls": [
            {
                "type": "function",
                "function": {
...
IMPORTANT: You must output your prediction in the following JSON format:
{
    ...
    "step3": {
        "role": "assistant",
        "content": "think: your reasoning process for step 3\\n\\naction: your actual action/tool call for step 3"
    }
}
...
"""
            ...
            format_instruction = """
**REQUIRED OUTPUT FORMAT:**
Your response MUST be a JSON array containing exactly THREE step objects:
```json
[
  {
    "role": "assistant",
  ...
  },
  {
    "role": "assistant",
    "content": "Observation: [describe screen state]\n\nThought: [reasoning for next action]",
    "action": "Scroll down on the [page/area] to reveal [something]."
  }
]
```

**OpenCUA Format Requirements:**
- MUST be a JSON array with exactly 3 step objects
...
- Example action: "Click on the grid icon (composed of small squares) in the top-right corner to open the Google apps menu."
- Do NOT use function call format like "click(element)" - use complete descriptive sentences
"""
        else:
            format_instruction = """
**REQUIRED OUTPUT FORMAT:**
Your response MUST be a JSON array containing exactly THREE steps.
...
**Your Task:**
Predict the NEXT THREE STEPS to progress toward solving this task.
You should predict THREE consecutive assistant actions/tool calls, WITHOUT including the tool execution results in between.
Think about what needs to be done next and plan the following three actions.
{format_instruction}
{json_only_instruction}
"""
        return prompt

    def predict_sample(self, sample: Dict) -> Optional[PredictionResultNext3]:
        """Generate next3 prediction for a single sample (predict next 2 assistant steps)."""
        try:
            ...
            print(f"\n{'=' * 80}")
            print(f"Predicting Next3: {task_id}")
            print(f"{'=' * 80}")
            ...
            if not reference_steps:
                print("⊘ Skipped: not enough steps for next3 prediction")
                return None

            # Generate prediction
            print("→ Generating next3 prediction...")
    ...
    def evaluate_prediction(
        self, prediction: PredictionResultNext3
    ) -> Optional[EvalResultNext3]:
        """Evaluate a next3 prediction (overall assessment of both steps)."""
        try:
            print(f"\n{'=' * 80}")
            print(f"Evaluating Next3: {prediction.task_id}")
            print(f"{'=' * 80}")
            ...
            print("→ Evaluating all three steps together as a whole...")
            ...
            eval_prompt = prompt_class.build_evaluation_prompt_next3(
                prediction.dataset,
                prediction.query,
            ...
            eval_data = self._parse_evaluation_next3(eval_response)
            if not eval_data:
                print("✗ Failed to parse next3 evaluation")
                return None

            result = EvalResultNext3(
    ...
    def _parse_evaluation_next3(self, response: str) -> Optional[Dict]:
        """Parse next3 evaluation response with json-repair fallback."""
        try:
            ...
            # Handle list response (aggregation)
            if isinstance(data, list):
                if not data:
                    print("⚠️  Received empty list in evaluation response")
                    return None

                print(f"  ℹ️  Received list of {len(data)} evaluations, aggregating...")
                print(f"{data}")
                # Aggregate results
                # 1. is_correct: True only if ALL are correct (YES/True)
                # 2. score: Minimum score (strict)
                # 3. errors: Union of all errors
                # 4. reasoning: Concatenate
                ...
                for i, item in enumerate(data):
                    ...
                    # Aggregate errors (OR logic)
                    for key in aggregated.keys():
                        if key.startswith("E") and item.get(key, False):
                            aggregated[key] = True

                    # Aggregate score (MIN logic for strictness)
                    item_score = item.get("score", 0.0)
                    if item_score < aggregated["score"]:
                        aggregated["score"] = item_score

                    # Aggregate reasoning
                    step_reasoning = item.get("reasoning", "")
                    if step_reasoning:
                        reasoning_parts.append(f"[Step {i + 1}] {step_reasoning}")

                aggregated["reasoning"] = "\n".join(reasoning_parts)

                # Recalculate is_correct based on aggregated errors
                has_any_error = any(
                    [
                        aggregated["E1_GOAL_MISALIGNMENT"],
                        aggregated["E2_PREMATURE_CONCLUSION"],
                        aggregated["E3_CONSTRAINT_VIOLATION"],
                        aggregated["E4_LOGIC_ERROR"],
                        aggregated["E5_TOOL_USE_ERROR"],
                        aggregated["E6_HALLUCINATION_ERROR"],
                    ]
                )
                aggregated["is_correct"] = not has_any_error
    ...
    def run_inference(
    ...
    ) -> List[PredictionResultNext3]:
        """Run inference mode for next3: generate next3 predictions only."""
        print(f"\n{'=' * 80}")
        print(f"Starting Next3 Inference: {self.dataset_name}")
        print(f"{'=' * 80}")
        ...
        print(f"→ Processing {len(dataset)} samples (next3 inference)")
        ...
        if self.resume:
            existing_predictions = self._load_latest_predictions_next3()
        ...
        for i, sample in enumerate(dataset, 1):
            ...
            if prediction:
                ...
                generated += 1
                # Auto-save
                self._save_predictions_incremental_next3(predictions)

        if generated > 0:
            self.save_predictions(predictions)

        print(f"\n{'=' * 80}")
        print(f"Completed: {len(predictions)} total next3 predictions")
    ...
    def run_evaluation(
        self, predictions: List[PredictionResultNext3]
    ) -> List[EvalResultNext3]:
        """Run evaluation mode for next3: evaluate existing next3 predictions."""
        print(f"\n{'=' * 80}")
        print(f"Starting Next3 Evaluation: {self.dataset_name}")
        print(f"{'=' * 80}")

        print(f"→ Evaluating {len(predictions)} next3 predictions")
        ...
        if self.resume:
            existing_results = self._load_latest_eval_results_next3()
            if existing_results:
        ...
        for i, prediction in enumerate(predictions, 1):
            ...
            if result:
                ...
                evaluated += 1
                # Auto-save
                self._save_results_incremental_next3(results)

        if evaluated > 0:
            self.save_results(results)

        print(f"\n{'=' * 80}")
        print(f"Completed: {len(results)} total next3 results")
    ...
    def run_pipeline(
        ...
        """Run pipeline mode for next3: inference + evaluation."""
        print(f"\n{'=' * 80}")
        print(f"Starting Next3 Pipeline: {self.dataset_name}")
        print(f"{'=' * 80}")
        ...
        print(f"→ Processing {len(dataset)} samples (next3 inference + evaluation)")
    ...
    def save_predictions(self, predictions: List[PredictionResultNext3]) -> str:
        """Save next3 predictions to file."""
        timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
        filename = f"{self.dataset_name}_predictions_next3_{timestamp}.json"
        ...
        print(f"\n✓ Next3 predictions saved to: {filepath}")

        return filepath

    def _save_predictions_incremental_next3(
        self, predictions: List[PredictionResultNext3]
    ) -> str:
        """Save next3 predictions incrementally (for resume mode)."""
        filename = f"{self.dataset_name}_predictions_next3_resume.json"
        ...
        print(f"  💾 Auto-saved {len(predictions)} next3 predictions to: {filepath}")

        return filepath

    def _load_latest_predictions_next3(self) -> List[PredictionResultNext3]:
        """Load latest next3 predictions for resume mode."""
        resume_file = os.path.join(
            self.prediction_resume_dir,
            f"{self.dataset_name}_predictions_next3_resume.json",
        )
        if os.path.exists(resume_file):
            try:
                return self._load_predictions_from_file_next3(resume_file)
            except Exception as e:
                print(f"  ⚠️  Failed to load resume file: {e}")

        return []

    def _load_predictions_from_file_next3(
        self, filepath: str
    ) -> List[PredictionResultNext3]:
        """Load next3 predictions from a specific file."""
        print(f"  📂 Loading next3 predictions from: {filepath}")
        ...
        for pred_dict in data.get("predictions", []):
            try:
                prediction = PredictionResultNext3(**pred_dict)
                predictions.append(prediction)
            except Exception as e:
    ...
    def load_predictions(self, filepath: str) -> List[PredictionResultNext3]:
        """Load next3 predictions from file."""
        predictions = self._load_predictions_from_file_next3(filepath)
    ...
    def _save_results_incremental_next3(self, results: List[EvalResultNext3]) -> str:
        """Save next3 evaluation results incrementally (for resume mode)."""
        filename = f"{self.dataset_name}_results_next3_resume.json"
        ...
        print(f"  💾 Auto-saved {len(results)} next3 evaluations to: {filepath}")

        return filepath

    def _load_latest_eval_results_next3(self) -> List[EvalResultNext3]:
        """Load latest next3 evaluation results for resume mode."""
        resume_file = os.path.join(
            self.resume_dir, f"{self.dataset_name}_results_next3_resume.json"
        )
        if os.path.exists(resume_file):
            try:
                return self._load_eval_results_from_file_next3(resume_file)
            except Exception as e:
                print(f"  ⚠️  Failed to load resume file: {e}")

        return []

    def _load_eval_results_from_file_next3(
        self, filepath: str
    ) -> List[EvalResultNext3]:
        """Load next3 evaluation results from a specific file."""
        print(f"  📂 Loading next3 evaluation results from: {filepath}")
        ...
        for result_dict in data.get("results", []):
            try:
                result = EvalResultNext3(**result_dict)
                results.append(result)
            except Exception as e:
    ...
    def save_results(self, results: List[EvalResultNext3]) -> str:
        """Save next3 results to file."""
        timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
        filename = f"{self.dataset_name}_results_next3_{timestamp}.json"
        ...
        print(f"\n✓ Next3 results saved to: {filepath}")
