...
for bench_name in "${TARGET_BENCHMARKS[@]}"; do
    ...
    if [ ! -f "offline_eval_without_reference.py" ]; then
        echo "❌ 错误: 在 ${bench_name} 中找不到 offline_eval_without_reference.py，跳过..."
        cd ".."
        continue
    fi
    ...
    srun --partition=ai_moe2 \
         ...
         --kill-on-bad-exit=1 \
         --quotatype=reserved \
         python offline_eval_without_reference.py
